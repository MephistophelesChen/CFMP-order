"""
Django settings for order service project.
"""
import os
import sys
...
# 添加公共模块路径
# 在容器中，common模块与order-service在同一级目录
...
INSTALLED_APPS = [
    ...
    'rest_framework',
    'corsheaders',
    'order.apps.OrderConfig',  # 使用应用配置类
]
...
# Database
DATABASES = {
    'default': DATABASE_CONFIG['ORDER_DB']
}
...
STATIC_URL = '/static/'
# 供 collectstatic 使用的目标目录（容器内会写入到 /app/staticfiles）
...
SERVICE_PORT = SERVICES['ORDER_SERVICE']['port']
SERVICE_VERSION = SERVICES['ORDER_SERVICE']['version']
